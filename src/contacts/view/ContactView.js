...
export class ContactView implements CurrentView {
	...
	headerView(): Children {
		...
		&& this._contactList.list.isMobileMultiSelectionActionActive() ? m(MultiSelectionBar, {
			selectNoneHandler: () => neverNull(this._contactList).list.selectNone(),
			selectedEntiesLength: this._contactList.list.getSelectedEntities().length,
			content: {
				view: () => m(ActionBar, {
					buttons: this._multiContactViewer.createActionBarButtons(() => neverNull(this._contactList).list.selectNone(), false)
				})
			}
		}) : null
	}
