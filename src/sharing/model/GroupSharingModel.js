...
import {getElementId, getEtId, getId, getLetId, isSameId} from "../../api/common/utils/EntityUtils"
import type {SentGroupInvitation} from "../../api/entities/sys/SentGroupInvitation"
import {createSentGroupInvitation, SentGroupInvitationTypeRef} from "../../api/entities/sys/SentGroupInvitation"
...
export class GroupSharingModel {
	...
	removeGroupMember(member: GroupMember): Promise<void> {
		return this.canRemoveGroupMember(member)
			...
			: Promise.reject(new Error("User does not have permission to remove this member from the group")) // TODO Error type
	}
	...
	entityEventsReceived(updates: $ReadOnlyArray<EntityUpdateData>, eventOwnerGroupId: Id): Promise<void> {
		return Promise.each(updates, update => {
			if (!isSameId(eventOwnerGroupId, getId(this.group))) {
